...
static int hf_rftap_bandwidth = -1;
static int hf_rftap_rxgain = -1;
    ...
    *subdissector_name = NULL;
    ...
    for (tag_id = 0; tag_id < 16; tag_id++, flag_bit<<=1) {
        ...
        case RFTAP_TAG_LOCATION:
            ...
            offset += 24;
            break;
        case RFTAP_TAG_RESERVED_13:
            float_val = tvb_get_letohieee_float(tvb, offset);
            proto_tree_add_float_format_value(tree, hf_rftap_bandwidth, tvb, offset, 4, float_val, "%.2f KHz", float_val);
            offset += 4;
            break;
        case RFTAP_TAG_RESERVED_14:
            float_val = tvb_get_letohieee_float(tvb, offset);
            proto_tree_add_float_format_value(tree, hf_rftap_rxgain, tvb, offset, 4, float_val, "%.2f dBi", float_val);
            offset += 4;
            break;
        default:
...
proto_register_rftap(void)
{
    ...
    static hf_register_info hf[] = {
        ...
        { &hf_rftap_bandwidth, {
            ...
            "rftap.bandwidth",
            FT_FLOAT, BASE_NONE, NULL, 0,
            "Signal bandwidth in Kilohertz", HFILL }},
        { &hf_rftap_rxgain, {
            "Receiver gain",
            "rftap.rxgain",
            FT_FLOAT, BASE_NONE, NULL, 0,
            "Receiver gain in dBi", HFILL }},
